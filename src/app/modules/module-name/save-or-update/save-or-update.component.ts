...
export class SaveOrUpdateComponent implements OnInit {
 ...
 private readonly notifier: NotifierService;
  constructor(
   ...
   if(data!==null){
     this.isEdit=true;
     this.idModuleName=data.id;
     const name=data.name;
    
    this.moduleNameForm.get("name").setValue(name);
  ...
  save() {
    ...
    let moduleName = new ModuleName();
    moduleName.id=this.idModuleName;
    moduleName.name = name;
    moduleName.organization = this.organization;
    if(this.moduleNameForm.valid){
    this.moduleNameService.saveOrUpdate(moduleName).subscribe( (resp) => {
  ...
  errormptyField(field: string) {
    return (
      this.moduleNameForm.get(field).hasError("required") &&
      (this.moduleNameForm.get(field).touched || this.isClickSave)
    );
  }
  invalidData(field: string) {
    return (
      this.moduleNameForm.get(field).hasError("whitespace") &&
      !this.moduleNameForm.get(field).hasError("required") &&
      (this.moduleNameForm.get(field).touched || this.isClickSave)
    );
  }
