...
import { MatDialog, MatTableDataSource, MatTable, Sort } from "@angular/material";
...
export class ListQuestionComponent implements OnInit {
  ...
  displayedColumns: string[] = ["note","indexNumerator","name", "actions"];
  ...
  onReCalculate(i) {
      ...
      for (let index = 0; index < this.questions.length; index++) {
        ...
        if(element.note ===0||element.note===null||this.existingIndexQuestion(index)){
          empty=true;
        }
      }
    ...
    invalidDataInArrayNote( i) {
      const note= this.questions[i].note;
      return (note!=null)&&(this.questions[i].note<0.25 ||this.questions[i].note>100)
      }
      ...
      invalidDataInArrayQuestion( i) {
        ...
        return (indexNumerator!=null)&&(this.questions[i].indexNumerator<1 ||this.questions[i].indexNumerator>this.getLenghtQuestions())
        }
        existingIndexQuestion(i){
          const indexNumerator= this.questions[i].indexNumerator;
          for (let index = 0; index < this.questions.length; index++) {
           
           if(index!=i && indexNumerator===this.questions[index].indexNumerator){
            return true;
           }
      ...
      onChangeIndexQuestion(i){
        ...
        for (let index = 0; index < this.questions.length; index++) {
          const element = this.questions[index];
          if(element.note ===0||element.note===null||this.existingIndexQuestion(index)||this.invalidDataInArrayQuestion(index)){
            empty=true;
          }
        }
      ...
      sortData(sort: Sort) {
        ...
        this.questions.sort((a, b) => {
          ...
          switch (sort.active) {
            ...
            case 'indexNumerator': return this.compare(a.indexNumerator, b.indexNumerator, isAsc);
            default: return 0;
          }
        });
      }
